/*
 * gdsp(1)  GTK Lattice Wave Digital Filters
 * 
...
static ssize_t __dsp_fp64_vec_realloc(struct dsp_fp64_freq * ffr, 
									   ssize_t npts)
{
	assert(ffr != NULL);
	assert(npts >= 2);
...
ssize_t dsp_fp64_freq_lin_set(struct dsp_fp64_freq * ffr, 
						   double w0, double w1, ssize_t npts)
{
	...
	__dsp_fp64_vec_realloc(ffr, npts);
...
ssize_t dsp_fp64_freq_log_set(struct dsp_fp64_freq * ffr, 
						   double w0, double w1, ssize_t npts)
{
	ssize_t cnt;
 ...
 */
struct dsp_fp64_freq * dsp_fp64_freq_new(struct dsp_fp64 * flt, 
										   size_t dftn)
{
	struct dsp_fp64_freq * ffr;
	complex double * z;
	size_t npts;
	double * wnd;
	double * w;
	...
	if ((ffr = calloc(1, sizeof(struct dsp_fp64_freq))) == NULL) {
		fprintf(stderr, "%s: calloc() failed: %s", __func__,
			strerror(errno));
		return NULL;
	};
...
void dsp_fp64_freq_free(struct dsp_fp64_freq * ffr)
{
	assert(ffr != NULL);
	assert(ffr->w != NULL);
	assert(ffr->z != NULL);
...
ssize_t dsp_fp64_lowwpass_freq_resp(struct dsp_fp64_freq * ffr,
									 struct dsp_fp64 * flt, 
									 double * pw[],
									 complex double * pz[])
{
...
ssize_t dsp_fp64_up_2x_freq_resp(struct dsp_fp64_freq * ffr,
								 struct dsp_filter * pre, 
								 struct dsp_filter * intp, 
								 double * pw[],
								 complex double * pz[])
{
#if FREQ_RESP_WND 
	...
	for (k = 0; k < n; ++k) {
		...
		while (rem) {
			if ((cnt = dsp_fp64_sink_push(sink, x, dftn)) < 0) {
				/* error */
				break;
			}
			rem -= cnt;

			while ((n = dsp_fp64_src_pull(src, y, buf_len)) < 0) {
				/* error */
				break;
			}
