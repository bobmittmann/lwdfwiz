...
struct plt_figure * plt_figure_new(const char * title, const char * xlabel, 
								   const char * ylabel)
{
	...
	figure->padding.left = 55.5;
	figure->padding.right = 20.5;
	figure->padding.top = 40.5;
	figure->padding.bottom = 40.5;
	...
	figure->opt.xauto = false;
	figure->opt.yauto = false;
...
#if (DEBUG > 2)
	fprintf(stderr, "scale: x=%g y=%g\n", figure->chart.scale.x, 
			figure->chart.scale.y);
#endif

	return 0;
}

int _plt_figure_span_fixup(struct plt_figure * figure)
{
	...
	if (figure->opt.xauto) {
		figure->axis.x.offs = min.x;
		figure->axis.x.span = max.x - min.x;
	}

	if (figure->opt.yauto) {
		figure->axis.y.offs = min.y;
		figure->axis.y.span = max.y - min.y;
	}
...
int plt_figure_xspan_auto(struct plt_figure * figure)
{
	...
	return _plt_figure_span_fixup(figure);
}
...
void plt_mk_lin_ticks(double pos[], char * label[], double from, double to, 
					  size_t cnt)
{
	double dv;
	double v;
